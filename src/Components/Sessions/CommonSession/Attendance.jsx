...
const TableData = ({ student }) => {
  const { name, attendanceStatus, reportStatus } = student;

  return (
    <div className="attendance-row">
      <li className="row-cell student-name">{name}</li>
      <li className="row-cell">
        <input type="checkbox" checked={attendanceStatus} disabled />
      </li>
      <li className="row-cell">
        <input type="checkbox" checked={reportStatus} disabled />
      </li>
    </div>
  );
};

const Attendance = ({ cohortUid, sessionId }) => {
    ...
    const fetchAttendanceData = async () => {
      try {
        const response = await callAPI(
          'get',
          `https://hfjgfndyj8.execute-api.us-east-1.amazonaws.com/testing/get_student_attendancea_and_report?cohortuid=${cohortUid}&sessionid=${sessionId}`
        );
            ...
            <div className="attendance-table">
              ...
              <div className="attendance-body">
                {attendanceData.map((student) => (
                  <TableData key={student.studentUsername} student={student} />
                ))}
              </div>
            </div>
            <div className="attendance-actions">
